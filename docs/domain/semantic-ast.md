...
`EmbeddedLanguageStatement(DLI)` conserva em `rawText` o slice normalizado desde
`EXEC` até o `END-EXEC` real, removendo somente o framing de transporte.
`hostOperands=[]`: a capability não determinou dependências internas, não provou
sua ausência. O finding é `PRESERVED_UNINTERPRETED/DEPENDENCY_UNKNOWN`.
Adjacência estrutural nas relações embedded não publica retorno normal; DLI não
integra `normalContinuations`. Não há interpretação de GU/GNP/REPL/SCHD/TERM.
