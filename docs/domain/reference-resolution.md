...
`RESOLVED` exige exatamente um candidate; `EXTERNAL_OBSERVED` não possui candidate; `AMBIGUOUS` conserva dois ou mais. `UNRESOLVED` e `UNSUPPORTED` carregam reason específico. O relatório combina binding, frontend coverage, COPYs, parser diagnostics e incertezas reais antes de afirmar readiness. Cada COPY não encontrado vira gap `UNRESOLVED_COPY` com nome e linha; o contador permanece no snapshot. `FrontendState` distingue esse input ausente dos erros estruturais que impedem o classifier. A observação de target literal externo não é, por si, uma lacuna.
