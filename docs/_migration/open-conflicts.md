...
| CONFLICT-003 | `specs/AGENTS.md — antlr-parse-tree-explorer.md`; código, specs e reports | O documento importado contém recomendações abrangentes, mas não foi instrução operacional histórica. | O próprio plano Harness v1 o classifica como `IMPORTED_ENGINEERING_KNOWLEDGE_SOURCE`. | Não promover texto automaticamente: validar cada regra contra código, contratos, testes e fontes de domínio antes de migrar. | OPEN_VALIDATION_REQUIRED |
