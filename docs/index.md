...
Os work items ativos estão no [índice de trabalho](work/index.md). `WORK-AST-002` mantém seu Slice 2 no checkpoint de Discovery do PR #13; implementar F-02 continua dependendo de merge/review e autorização explícita posterior. `WORK-COND-005` mantém o Slice 5 de `BACKLOG-COND-001` somente no checkpoint de Discovery; implementação de occurrences contextuais continua não autorizada. `WORK-COND-001` foi concluído pelo PR #15, `WORK-COND-002` pelo PR #16, `WORK-COND-003` pelo PR #17 e `WORK-COND-004` pelo PR #18; todos possuem apenas [resumos históricos](work/history/WORK-COND-001.md). `WORK-AST-003` foi concluído pelos PRs #11/#12 e possui apenas um [resumo histórico](work/history/WORK-AST-003.md).
