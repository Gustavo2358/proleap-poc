...
Os work items ativos estão no [índice de trabalho](work/index.md). `WORK-AST-002` mantém seu Slice 2 no checkpoint de Discovery do PR #13; implementar F-02 continua dependendo de merge/review e autorização explícita posterior. `WORK-COND-003` executa o Slice 3 de `BACKLOG-COND-001`: torna a condition surface lossless na AST, sem binding, sem `ConditionSemantics` e sem validação type-sensitive. `WORK-COND-001` foi concluído pelo PR #15 e `WORK-COND-002` pelo PR #16; ambos possuem apenas [resumos históricos](work/history/WORK-COND-001.md). `WORK-AST-003` foi concluído pelos PRs #11/#12 e possui apenas um [resumo histórico](work/history/WORK-AST-003.md).
