...
#### Round 2 — correções do review humano (este round; findings F1–F4)
...
3. Introduzir helpers puros de policy: `relationOperandKinds(ref)` (bare `{DATA, INDEX}`; qualified/subscripted `{DATA}`) e `contextualKinds(ref) = relationOperandKinds(ref) UNIÃO {CONDITION}`, ambos sobre `DataReference.qualifiers()`/`subscriptGroups()`.
...
9. Gates + semantic challenge pass (R2-1..R2-10 e CO-01..CO-20).
...
- Slices 3 e 4 já fornecem containers de condition surface e `DataReference` lossless com qualifiers/subscriptGroups tipados.
...
- Review humano deste Discovery Round 2 e autorização explícita são pré-condições do Checkpoint 2.
...
| `ReferenceOccurrenceCollector.java` | helper estrutural shape-sensitive (`relationOperandKinds(ref)`/`contextualKinds(ref)`) para condition surface; remover equality por `grammarRule` | sem lookup, sem materializar predicate, sem contaminação de children |
...
- `ContextualConditionOccurrenceDiscoveryTest`: manter FACTs R2-01..R2-04, NOT oracle, children independentes e o que provar os NEG oracles shape-sensitive.
...
- `ContextualConditionOccurrenceTest.java` com CO-01..CO-20 e NEG-INDEX-QUAL-01, NEG-INDEX-SUB-01, NEG-CONTEXT-INDEX-QUAL-01, NEG-CONTEXT-INDEX-SUB-01;
