...
7. **Checkpoint 4 — Publicação no composition root e consumer independente.**
   ...
   CLI/JSON ou adicionar lowerer. Exercitar determinismo por valor, ordering,
   provenance, policy e fechamento do lifecycle.
8. **Review humano obrigatório e encerramento do slice.** Executar os gates do
   ...
   item. O encerramento não autoriza generalização, interchange, CFG/dataflow ou
   qualquer capability posterior.
...
- O contrato aprovado nos Checkpoints 2, 3A e 3B de
  `WORK-SEMANTIC-PRODUCT-001`, especialmente a recomendação H e o oracle
...
preservar os produtos de entrada, manter a apresentação como adapter separado e
evitar tornar o produto dependente do lifecycle do `ExplorerMain`. Não haverá
serializer, schema, round-trip, persistência ou refatoração transversal como
