...
- IBM Enterprise COBOL for z/OS 6.4 Language Reference, **Abbreviated combined relation conditions**: https://www.ibm.com/docs/en/cobol-zos/6.4.0?topic=expressions-abbreviated-combined-relation-conditions
- IBM Enterprise COBOL for z/OS 6.4 Language Reference, **Complex conditions**: https://www.ibm.com/docs/en/cobol-zos/6.4.0?topic=expressions-complex-conditions
- IBM Enterprise COBOL for z/OS 6.4 Language Reference, **General relation conditions** e **Comparison of index-names and index data items**: https://www.ibm.com/docs/en/cobol-zos/6.4.0?topic=conditions-comparison-index-names-index-data-items
- IBM Enterprise COBOL for z/OS 6.4 Language Reference, capítulos **User-defined words** e **Referencing data names... / Condition-name / Index-name**: https://publibfp.dhe.ibm.com/epubs/pdf/igy6lr40.pdf
- Para nested programs e visibilidade, IBM Enterprise COBOL for z/OS 6.5 Programming Guide, **Scope of names**: https://www.ibm.com/docs/en/SS6SG3_6.5/pdf/pgmvs.pdf
...
8. **`NOT` lógico**: nega apenas a relation/simple condition imediatamente seguinte e não é herdado com sujeito/operador.
...
13. **Scope/visibility**: nomes locais e globais de nested programs obedecem ao scope IBM; uma declaração local aplicável não pode ser ignorada em favor de um homônimo de programa contendo por conveniência de namespace.
...
- `A NOT = B OR C` trata `NOT =` como relational operator; `A = B OR NOT C` trata `NOT` como lógico somente se `C` constituir condition válida nessa posição.
...
- Qualification e scope são aplicados pelas regras IBM antes de qualquer seleção conveniente de candidato. Ambiguidade real deve permanecer ambiguidade; input inválido deve permanecer inválido.
