...
`ConditionSemantics` também ainda não existe em produção. ADR-0012 propõe esse produto imutável entre resolução nominal e consumidores de predicates para especializar condições cujo significado depende do binding, sem reescrever AST/occurrences/resolution. Enquanto o ADR e os slices executáveis não forem aprovados, o pipeline corrente permanece o diagrama acima e a lacuna continua explícita.
