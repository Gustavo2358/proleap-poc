...
final class PreprocessorEngine {
    ...
    Outcome process(String normalized, String file) {
        ...
        int[] unresolved = {0};
        String text = processRecursive(normalized, file, diagnostics, unresolved, new HashSet<>());
        ...
        return new Outcome(text, Math.toIntExact(errors), unresolved[0], List.copyOf(diagnostics));
    }
        ...
        for (ParserRuleContext context : actionable) {
            ...
            if (rule.equals("copyStatement")) {
                ...
                if (path.isEmpty()) {
                    ...
                    try {
                        String copyText = processRecursive(library.readNormalized(path.get()), path.get().getFileName().toString(),
                                diagnostics, unresolved, expansionStack);
                        Matcher replacements = PSEUDO_REPLACE.matcher(original);
                        while (replacements.find()) copyText = copyText.replace(replacements.group(1).trim(), replacements.group(2).trim());
