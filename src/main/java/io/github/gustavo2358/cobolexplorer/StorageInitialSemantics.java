...
public final class StorageInitialSemantics {
    ...
    public enum Kind { LITERAL_BYTES, POSSIBLE_LITERAL_BYTES, POSSIBLE_LOGICAL_TEXT, LOGICAL_TEXT, PRESERVE, UNKNOWN }
    ...
    public enum Reason { ENTRY_STATE_NOT_PROVEN, STORAGE_NOT_PROVEN, VALUE_NOT_PROVEN, VALUE_ON_REDEFINITION, NESTED_VALUE,
        STORAGE_NOT_LOCAL, INCOMPLETE_WRITE_INVENTORY, OVERLAPPING_WRITE, WRITE_NOT_PROVEN, UNKNOWN_STORAGE_EFFECT, FOREIGN_MUTATION_OR_ESCAPE, SOURCE_PROVENANCE_NOT_PROVEN }
    ...
    static StorageInitialSemantics analyze(CompilationUnitBuildResult frontend,ReferenceResolution resolution,StorageLayoutSemantics layout,EntryMode mode,
            Map<Key,StorageAccessSemantics.Access> accesses,Map<Key,List<StorageAccessSemantics.Move>> moves,Map<Key,StatementEffectSummary> effects,CicsProgramControlAnalyzer.Contribution cics,FileIoMemory files) {
        ...
        for(var unit:frontend.compilationUnit().programUnits()) {
            ...
            var inventory=mode==EntryMode.UNKNOWN&&!unit.program().attributes().initial()
                ?StorageMutationInventory.analyze(frontend,unit,physical,accesses,moves,effects,cics,files):null;
            ...
            while(!pending.isEmpty()) {
                ...
                // FILE buffer content is supplied by I/O, outside the source VALUE entry profile.
                if(node instanceof Ast.FileDescription)continue;
            ...
            for(var data:declarations) {
                // Nonphysical declarations remain outside this entry profile.
                ...
                boolean exactOrigin=values.get(0).meta().provenance().exact();
                if(!exactOrigin)reasons.add(Reason.SOURCE_PROVENANCE_NOT_PROVEN);
                boolean invalidSource=!exactOrigin||flags.redefined()||flags.value()||descendantValues.get(data.meta().id())>values.size()
