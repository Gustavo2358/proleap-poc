...
public final class FileAuxiliarySemantics {
    ...
    public record FileRef(ResolutionStatus status,List<SemanticEntityId> candidates,Ast.SourceProvenance origin){public FileRef{candidates=List.copyOf(candidates);}}
    ...
    static FileAuxiliarySemantics analyze(CompilationUnitBuildResult frontend,ReferenceResolution resolution,CompilationUnitSymbolTables tables){
        ...
        for(var unit:frontend.compilationUnit().programUnits()) {
            ...
            for(var node:nodes.values()){
                ...
                if(subject!=null)files.add(new FileRef(ResolutionStatus.RESOLVED,List.of(subject),c.meta().provenance()));
                for(var ref:c.files()){var b=bindings.get(new Key(unit.id(),ref.meta().id()));files.add(new FileRef(b==null?ResolutionStatus.UNRESOLVED:b.status(),b==null?List.of():b.candidates().stream().map(ReferenceResolution.Candidate::entityId).toList(),ref.meta().provenance()));}
                ...
                if(c.kind()==Ast.FileAuxKind.SAME_AREA){boolean vsam=!targets.isEmpty()&&targets.size()==files.size()&&targets.stream().allMatch(id->vsam(controls.get(id)));
                    effect=vsam?Effect.RECORD_ALIAS:Effect.CONDITIONAL_RECORD_ALIAS;if(!vsam)gaps.add("SAME_AREA_ACCESS_METHOD_NOT_PROVEN");}
                ...
                if(c.kind()==Ast.FileAuxKind.LINAGE&&effect!=Effect.DOCUMENTARY){if(subject!=null&&vsam(controls.get(subject)))gaps.add("LINAGE_REQUIRES_SEQUENTIAL_FILE");}
                ...
                if(c.kind()==Ast.FileAuxKind.RERUN){
                    ...
                    for(var id:targets){var control=controls.get(id);var fd=descriptions.get(id);
                        if(!id.programUnitId().equals(unit.id())||control==null||fd==null||fd.visibility()==Ast.DeclarationVisibility.EXTERNAL||vsam(control)&&control.control().organization()!=Ast.FileOrganization.SEQUENTIAL)gaps.add("RERUN_TRIGGER_FILE_RESTRICTION");}
                }
    ...
    static boolean vsam(Ast.FileBinding f){return f!=null&&f.control()!=null&&(f.control().organization()==Ast.FileOrganization.INDEXED||f.control().organization()==Ast.FileOrganization.RELATIVE);}
