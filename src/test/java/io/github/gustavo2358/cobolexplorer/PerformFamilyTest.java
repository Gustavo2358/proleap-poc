...
class PerformFamilyTest {
    ...
    @Test void thruRangeIdentitiesCompletionAndIndependentResumes() throws Exception {
        ...
        var facts=ranges(sp);assertEquals(2,facts.size());assertEquals("2.2.0",sp.path("contractVersion").asText());
        for(var p:facts) {
    ...
    @Test void allThruFocalsAdversarialAndMultiplicity() throws Exception {
        ...
        try(var files=Files.list(root)) {
            for(var path:files.filter(p->p.toString().endsWith(".cbl")).sorted().toList()) {
