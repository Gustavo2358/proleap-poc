...
class FileEffectsContractTest {
    ...
    @Test void nativeReadPublishesConditionalEffectsAndGeneralAllocationProof()throws Exception {
        ...
        var p=publish(f);assertEquals("2.25.0",p.path("contractVersion").asText());assertEquals("1.8.0",p.path("storage").path("version").asText());
        assertEquals("1.4.0",p.path("fileInventory").path("version").asText());var e=effect(p);assertEquals("KNOWN",e.path("availability").asText());
